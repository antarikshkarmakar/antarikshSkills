...
### IV. Terse Communication (Caveman Style)
...
Initialize a repository, setup `memory/` (4-layer memory system folders), and create initial `INTERFACES.md` contract.

### `/compact` — Memory Consolidation
Write step at session end: update `SEMANTIC.md` (facts), append to `EPISODIC.md` (history), clear `WORKING.md`. (If toolless: print updated SEMANTIC and EPISODIC for the user to save).

---

## 4. 4-Layer Memory Protocol

Always read and maintain files in `memory/`:
1. **`memory/IDENTITY.md`** -- who you are.
2. **`memory/SEMANTIC.md`** -- durable project facts.
3. **`memory/EPISODIC.md`** -- append-only log of session events.
4. **`memory/WORKING.md`** -- current session context.

- **Start of Session**: Read all four files. Introduce yourself with a 1-paragraph summary. Ask if there are new updates.
- **End of Session**: Write to `SEMANTIC.md` (facts), append to `EPISODIC.md` (history), clear `WORKING.md`.
