...
| **`graphify`** | `ak-grok` | Optional | Check skill config directory or `graphify --help` | `pip install graphifyy` then `graphify install` after reviewing the package; project installer can prompt for this with `--install-optional` / `-InstallOptional` | Falls back to CodeGraph or manual parsing. |
...
| **`headroom`** | `ak-headroom`, `RULESET.md` Cache Optimization | Optional | `headroom --version` | `uv tool install "headroom-ai[all]"` or `pip install "headroom-ai[all]"` for the CLI; npm package is SDK-only; project installer can prompt for this with `--install-optional` / `-InstallOptional` | Skip reversible context compression; fall back to normal text processing. |
