...
2. **Install the Plugin**:
   ...
   /plugin install antariksh-skills
   ```
   This registers the 25 modular skill guides globally inside Claude Code.
...
This repository includes Codex-native plugin metadata in `.codex-plugin/plugin.json` and a repo marketplace catalog at `.agents/plugins/marketplace.json`. The plugin package exposes the 25 modular `ak-*` skills through `skills/`; the project installer remains the path for generating per-repo rule files, memory scaffolding, and opt-in hooks.
...
```
This registers the master `antariksh-unified-skill` and the modular commands (`/ak-grill`, `/ak-align`, `/ak-align-docs`, `/ak-to-prd`, `/ak-spec`, `/ak-tdd`, `/ak-verify`, `/ak-diagnose`, `/ak-bughunt`, `/ak-devops`, `/ak-ci-check`, `/ak-security`, `/ak-skillset`, `/ak-code`, `/ak-review`, `/ak-prreview`, `/ak-worktree`, `/ak-orchestrate`, `/ak-doc`, `/ak-grok`, `/ak-audit-arch`, `/ak-scratch`, `/ak-compact`, `/ak-handoff`, `/ak-headroom`) in your active agent environments.
...
This repository is compatible with [skills.sh](https://www.skills.sh/) as both a root master skill and a modular skill collection. The repo root exposes `antariksh-unified-skill`; the `skills/` directory exposes the 25 focused `ak-*` skills. For public discovery, prefer installing the modular collection:
...
- **graphify**: if found as a Claude skill folder or `graphify` CLI, `/ak-grok` uses it to build a real knowledge graph of the repo (`graphify-out/`). If not found, `/ak-grok` checks for Understand-Anything next, then falls back to a manual directory/stack scan.
...
- For supply-chain safety, this framework does not print or execute raw third-party installer URLs. `install.ps1 -InstallOptional` and `install.sh --install-optional` are explicit opt-in paths: they prompt before installing Graphify, Caveman, CodeGraph, Sentry CLI, and Headroom. The default installer path remains read-only detection.
- Optional install commands use reviewed package-manager paths only: `uv tool install graphifyy` or `pipx install graphifyy` + `graphify install`, `claude plugin marketplace add JuliusBrussee/caveman` + `claude plugin install caveman@caveman`, `npm install -g @colbymchenry/codegraph` + `codegraph install`, `npm install -g sentry`, and `uv tool install "headroom-ai[all]"`, `pipx install "headroom-ai[all]"`, or `pip install "headroom-ai[all]"`.
- When the shell is already inside a Python virtualenv, the pip fallback intentionally omits `--user` because Python rejects user-site installs from most virtualenvs.
- Sentry authentication is never performed by the installer. After installing the CLI, run `sentry auth login` only when you want telemetry-backed diagnosis. Headroom MCP/proxy configuration stays under `/ak-headroom` after the CLI is installed.
...
| **`/ak-align`** | Pre-coding Socratic scope alignment to confirm goals, constraints, non-goals, and establish a strict implementation plan gate. Pushes every success criterion until it is binary and observable — a criterion that cannot fail has not been agreed, only postponed. | [skills/align/SKILL.md](file:///c:/GitHub/antarikshSkills/skills/align/SKILL.md) |
...
| **`/ak-tdd`** | MATT POCOCK Test-Driven Development (RED-GREEN-REFACTOR) cycle, bootstrapping minimal test setups if needed. Iron Law (no production code without a failing test), right-reason RED verification, and delete-and-restart for code written before its test. | [skills/tdd/SKILL.md](file:///c:/GitHub/antarikshSkills/skills/tdd/SKILL.md) |
| **`/ak-verify`** | Verification before completion — the evidence gate every other skill's *Evidence Over Claims* footer points to. Iron Law (no completion claims without fresh verification evidence), a programmatic-check-over-opinion ordering, an author-intent-bias guard (judge the artifact, not what you meant it to do), a claim→required-evidence table, and an honest path for claims that genuinely cannot be verified locally. | [skills/verify/SKILL.md](file:///c:/GitHub/antarikshSkills/skills/verify/SKILL.md) |
| **`/ak-diagnose`** | Structured debugging loop (READ FIRST: error text, own stack frames, recent diff → REPRODUCE via Sentry/logs → MINIMIZE via bisection → 5-WHYS root cause → FIX & PREVENT), gated by the Iron Law: no fixes without root cause investigation first. | [skills/diagnose/SKILL.md](file:///c:/GitHub/antarikshSkills/skills/diagnose/SKILL.md) |
| **`/ak-bughunt`** | High-severity bug hunt on recent commits: trace caller chains (never pattern-match diffs), patch only with a concrete trigger scenario + regression test, one-line all-clear otherwise. | [skills/bughunt/SKILL.md](file:///c:/GitHub/antarikshSkills/skills/bughunt/SKILL.md) |
...
| **`/ak-skillset`** | Skill authoring gated on evidence: a **RED baseline** (without the skill the task must fail outright, or succeed without evidence/scoping/record — no failure of either kind means no skill) and a **GREEN check** (behaviour must change *with* it). Then observation intake, triage classes, 11 thinking lenses, XML specs that never contain ungrounded sections, public/internal safety sweep, and a multi-agent synthesis loop. | [skills/skillset/SKILL.md](file:///c:/GitHub/antarikshSkills/skills/skillset/SKILL.md) |
| **`/ak-code`** | Surgical code implementation using the Ponytail ladder (Native first, standard library, YAGNI), with task right-sizing (a unit is the smallest piece carrying its own test cycle and worth a reviewer's separate verdict) and a no-placeholders rule — stubs and TODOs are never handed back as finished work. | [skills/code/SKILL.md](file:///c:/GitHub/antarikshSkills/skills/code/SKILL.md) |
| **`/ak-review`** | Adversarial proposer-attacker duel against edge cases, race conditions, and security surfaces. The attacker reads the diff alone, never the intention behind it — the next reader gets the diff and nothing else. | [skills/review/SKILL.md](file:///c:/GitHub/antarikshSkills/skills/review/SKILL.md) |
...
| **`/ak-worktree`** | Manages Git Worktrees for concurrent, isolated task execution: records a clean test baseline at creation (so your breakage is distinguishable from pre-existing), then gates the finish on an explicit merge/PR/keep/discard choice before any destructive cleanup. | [skills/worktree/SKILL.md](file:///c:/GitHub/antarikshSkills/skills/worktree/SKILL.md) |
| **`/ak-orchestrate`** | Fleet orchestration, gated on verifiable success and declared budgets: plan → decompose → **on-disk ledger** (survives compaction, prevents re-dispatching finished work) → **pre-flight baseline** (never fan out onto an already-red tree) → self-contained briefs → delegate down with explicit per-child models → two-stage review by *differing* reviewers (spec compliance, then code quality) with a capped escalating fix loop → synthesize a unified PR. Refuses to split work whose hard part is coherence, and reports budget exhaustion honestly rather than dressing partial work as complete. | [skills/orchestrate/SKILL.md](file:///c:/GitHub/antarikshSkills/skills/orchestrate/SKILL.md) |
