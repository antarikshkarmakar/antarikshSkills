...
- Optional install commands use reviewed package-manager paths only: `pip install graphifyy` + `graphify install`, `claude plugin marketplace add JuliusBrussee/caveman` + `claude plugin install caveman@caveman`, `npm install -g @colbymchenry/codegraph` + `codegraph install`, `npm install -g sentry`, and `uv tool install "headroom-ai[all]"` or `pip install "headroom-ai[all]"`.
