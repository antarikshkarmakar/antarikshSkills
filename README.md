...
| **`/ak-verify`** | Verification before completion — the evidence gate every other skill's *Evidence Over Claims* footer points to. Iron Law (no completion claims without fresh verification evidence), a claim→required-evidence table, and an honest path for claims that genuinely cannot be verified locally. | [skills/verify/SKILL.md](file:///c:/GitHub/antarikshSkills/skills/verify/SKILL.md) |
...
| **`/ak-code`** | Surgical code implementation using the Ponytail ladder (Native first, standard library, YAGNI), with task right-sizing: a unit is the smallest piece carrying its own test cycle and worth a reviewer's separate verdict. | [skills/code/SKILL.md](file:///c:/GitHub/antarikshSkills/skills/code/SKILL.md) |
...
| **`/ak-orchestrate`** | Fleet orchestration, gated on verifiable success and declared budgets: plan → decompose → **on-disk ledger** (survives compaction, prevents re-dispatching finished work) → self-contained briefs → delegate down with explicit per-child models → two-stage review by *differing* reviewers (spec compliance, then code quality) with a capped escalating fix loop → synthesize a unified PR. Refuses to split work whose hard part is coherence, and reports budget exhaustion honestly rather than dressing partial work as complete. | [skills/orchestrate/SKILL.md](file:///c:/GitHub/antarikshSkills/skills/orchestrate/SKILL.md) |
