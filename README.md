...
| **`/ak-orchestrate`** | Fleet orchestration: plan → decompose → **on-disk ledger** (survives compaction, prevents re-dispatching finished work) → self-contained briefs → delegate down with explicit per-child models → two-stage review (spec compliance, then code quality) with a capped escalating fix loop → synthesize a unified PR. | [skills/orchestrate/SKILL.md](file:///c:/GitHub/antarikshSkills/skills/orchestrate/SKILL.md) |
