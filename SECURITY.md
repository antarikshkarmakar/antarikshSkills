...
- **No default auto-install**: skills never install software on the user's behalf. The project installers only install supported third-party accelerators when the user explicitly passes `--install-optional` / `-InstallOptional` and confirms each prompt. Missing tools otherwise produce a warning pointing to [DEPENDENCIES.md](DEPENDENCIES.md) install hints (Philosophy VIII).
