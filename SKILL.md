...
- **Don't over-apply this**: Ask only when the ambiguity would change the approach or outcome. Routine implementation details don't need a question — that's what Philosophy V (Caveman terse, autonomous execution) is for.
...
4. **Mid-task scope changes**: If new requirements surface after work has started, classify the change before continuing instead of silently absorbing it as "while I'm at it" work:
   ...
   - **Hold Scope** — defer it: note it as an open loop (Second Brain Loop) and finish the agreed scope first.
...
Follow a strict Test-Driven Development flow. Do not write implementation code first. If no test framework exists yet, bootstrap the minimal one for the stack first (Ponytail Ladder — stdlib or an already-installed dependency before adding a new one); don't skip RED-GREEN-REFACTOR just because nothing was there to begin with.
...
3. **Attacker Phase**: Switch context entirely. Assume the Proposer is wrong. Attack on the axes that apply: Edge Cases, Race Conditions, Silent Failures, Assumption Violations, Security Surfaces, and Classic Bugs.
...
2. **Check for a knowledge-graph tool.** Look for graphify (`SKILL.md` under `<home>/.claude/skills/graphify/`, `<home>` = `~` on macOS/Linux/WSL, `%USERPROFILE%` on Windows) or Understand-Anything (a `.claude-plugin/`/`.understand-anything/` marker, or its `/understand` command being available). The installer records graphify's status in `MEMORY.md` under "Context Agent Needs" — trust that note instead of re-probing the filesystem every time.
3. **If either is available**: delegate to it (incrementally, if it supports re-running on just the changed files) to build/update the knowledge graph. Summarize the result as the repo map: stack, module boundaries, and entry points. Don't run both — pick whichever is detected first.
4. **If neither is available**: fall back to a manual scan — on a fresh repo, walk the directory tree, identify the stack from manifest files (`package.json`, `pyproject.toml`, `go.mod`, `*.csproj`, etc.), locate the test framework and entry points; on a previously-scanned repo, only re-walk the files from step 1's diff.
...
1. **Check for pathfinder**: if a codebase-mapping skill (e.g. `claude-mem:pathfinder`) is available, delegate to it — map the codebase into feature-grouped flowcharts, identify duplicated concerns, and propose a unified architecture. Don't reinvent it (Ponytail rung 2).
...
- Scaffold the **Second Brain System** folders (`memory/`, `memory/daily/`, `memory/projects/`, `memory/adr/`, `memory/prds/`, `MEMORY.md`, `AGENTS.md`, `GLOSSARY.md`, `inbox.md`, `memory/handoff.md`).
- Create `INTERFACES.md` to define initial module boundaries.
- Per Philosophy VI (Security & Credential Protection): create a `.gitignore` covering secrets (`.env`, keys) and common build/dependency junk if none exists, or append the missing entries if one already exists without them.

### `/compact` — Memory Consolidation
Trigger a write step to compress session learnings into the Second Brain system. If the `caveman` plugin is installed, run `/caveman-compress` on the updated memory files as the final step.
...
## 4. The Second Brain Loop (Continuous Context)
