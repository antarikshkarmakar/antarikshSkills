...
$ruleHeaders = @{
    ...
    ".clinerules/01-antariksh-framework.md" = "# Cline/Roo-Code System Rules (.clinerules)`n`nYou are an expert developer assistant executing within Cline or Roo-Code. You follow the **Antariksh Unified Developer Framework**.`n`n---`n`n"
    ...
    "SKILL.md"     = "---`nname: antariksh-unified-skill`ndescription: Master developer skill combining planning, simplicity, TDD, diagnosis, devops, QA, security, and skill evolution`n---`n`n# Antariksh Unified Agent Skill (Master Developer Framework)`n`nThis is a master-skill for developer agents. When running in a toolless or web-UI interface, follow the inline loops and command workflows below.`n`n## 1. Core Sessions Loop`n- **Session Start**:`n  1. Read ``memory/handoff.md`` if exists → then delete/clear it.`n  2. Read ``MEMORY.md``.`n  3. Read ``memory/local_env.md`` if exists (local skills/tools).`n  4. Read ``AGENTS.md`` + ``GLOSSARY.md``.`n  5. **Context Validation Check**: Check if ``memory/projects/<name>.md`` exists. If not, alert the user and advise running ``/ak-grok`` first to build the project context card and knowledge graph.`n  6. **Episodic Review**: Read the last 5 daily logs (``memory/daily/*.md``) to gain historic execution context.`n  7. **Session Boot**: Set up today's daily log and ask the user `"Is there anything new or changed before we begin?`"`n- **Session End**: Run ``/ak-compact`` to summarize logs, update project lists, update MEMORY.md, record learned corrections, and append reusable skill observations to ``memory/skill-observations.md``.`n`n## 2. Slash Commands Index & Workflows`n- **``/ak-grill``**: Interrogate scope, check edge cases, and output action plan → ``.agents/skills/grill/SKILL.md``.`n- **``/ak-align``**: Pre-coding Socratic scope alignment to agree on plans and success criteria; push each criterion until it is binary and observable.`n- **``/ak-align-docs``**: Scope alignment + Shared Language glossary update + ADR generation → ``.agents/skills/align-docs/SKILL.md``.`n- **``/ak-to-prd``**: Scopes features with module quizzes and drafts PRD to ``memory/prds/`` → ``.agents/skills/to-prd/SKILL.md``.`n- **``/ak-spec``**: Spec-driven loop (specify -> clarify -> plan -> tasks -> analyze -> implement -> converge) → ``.agents/skills/spec/SKILL.md``.`n- **``/ak-tdd``**: Test-driven development. Iron Law: no production code without a failing test first; RED must fail for the right reason; delete code written before its test.`n- **``/ak-verify``**: Verification before completion. Iron Law: no completion claims without fresh verification evidence; identify the proving command, run it in full, read the output, then claim → ``.agents/skills/verify/SKILL.md``.`n- **``/ak-diagnose``**: Read error text and recent diff first -> reproduce -> bisect scope -> 5-whys root cause -> surgical fix -> prevent. Iron Law: no fixes without root cause investigation first.`n- **``/ak-bughunt``**: Sweep recent commits for critical defects (trace callers -> concrete trigger scenario -> minimal gated fix or one-line all-clear) → ``.agents/skills/bughunt/SKILL.md``.`n- **``/ak-devops``**: Scaffold container/IaC files, run linters, validate dry-run setups.`n- **``/ak-ci-check``**: Run local line ending, shellcheck, Trivy scan, secrets scan, and indentation diff checks.`n- **``/ak-security``**: OWASP threat audit, local credentials scan, dependency CVE audit, and security report.`n- **``/ak-skillset``**: RED baseline (skill must fix a capability failure or a rigour failure; neither = no skill) -> observation intake -> skill triage (USE_EXISTING, etc.) -> 11 lenses analysis -> XML spec, no ungrounded sections -> GREEN check on behaviour -> safety sweep -> critique duel.`n- **``/ak-code``**: Surgical minimal implementation (contracts check -> right-size units -> lazy ladder -> tests -> diff check). Ship no placeholders or TODO stubs as finished work.`n- **``/ak-review``**: Adversarial attacker duel against edge cases and interface drift. Attack the diff, not the intention behind it.`n- **``/ak-prreview``**: Gated PR review creating draft reviews for explicit user approval.`n- **``/ak-worktree``**: Worktree-isolated parallel subagent sweep orchestration. Record a clean test baseline at creation; finish with an explicit merge/PR/keep/discard choice before destructive cleanup.`n- **``/ak-orchestrate``**: Fleet orchestration (gate on verifiable success -> plan + budget -> decompose -> ledger -> pre-flight baseline -> brief -> delegate -> two-stage review by differing reviewers -> synthesize). Do not delegate what you cannot verify; do not split work whose hard part is coherence; report budget exhaustion honestly → ``.agents/skills/orchestrate/SKILL.md``.`n- **``/ak-doc``**: Direct module and interface documentation via tables and diagrams → ``.agents/skills/doc/SKILL.md``.`n- **``/ak-grok``**: Incremental repository scans (RAG index building/AST parsing) to map structure.`n- **``/ak-audit-arch``**: Sweep codebase for architectural smells (god files, duplicate logic, tangles).`n- **``/ak-scratch``**: Scaffold new projects with standard folder layouts and template configs → ``.agents/skills/scratch/SKILL.md``.`n- **``/ak-compact``**: Log consolidation, project facts compilation, skill-observation capture, inbox clearing, and corrections capture.`n- **``/ak-handoff``**: Compile handoff notes to ``memory/handoff.md`` for incoming agents.`n- **``/ak-headroom``**: Detect and configure Headroom for reversible token compression (MCP/proxy); skip cleanly when it is not installed → ``.agents/skills/headroom/SKILL.md``.`n"
}
...
$legacyClineRules = Join-Path $targetPath ".clinerules"
if ((Test-Path $legacyClineRules) -and !(Get-Item $legacyClineRules).PSIsContainer) {
    Move-Item -Path $legacyClineRules -Destination (Join-Path $targetPath ".clinerules.legacy.bak") -Force
    Write-Host "Moved legacy .clinerules file to .clinerules.legacy.bak (Cline now reads the .clinerules/ directory). Delete the backup once you have copied over any local edits." -ForegroundColor Yellow
}
...
$claudeRulesDir = Join-Path $targetPath ".claude/rules"
$claudeRulesReadme = Join-Path $claudeRulesDir "README.md"
if (!(Test-Path $claudeRulesReadme) -or $Force) {
...
'@
    Set-Content -Path $claudeRulesReadme -Value $claudeRulesBody -Encoding utf8
    Write-Host "Scaffolded: .claude/rules/README.md" -ForegroundColor Green
} else {
    Write-Host "Skipped: .claude/rules/README.md (already exists, use -Force to overwrite)" -ForegroundColor Yellow
}
